...
- CSV 价格样本导入和管理，支持把用户自有或授权的成交样本加入本地相似商品库，并停用、恢复、备注、删除、查看或导出操作审计。
...
- `GET /api/v1/market-data/audit`：查看价格样本操作审计，可按 `item_id`、`action` 筛选。
- `GET /api/v1/market-data/audit/export`：导出价格样本审计，支持 `format=csv` 或 `format=markdown`。
...
测试覆盖字段检查、估价规则、相似商品检索、本地图片线索、授权价格样本导入/管理/审计导出、库存和发布表现、一条完整 API 链路和六个固定演示案例。测试数据库会放在临时目录，不会污染 `data/resale.db`。
