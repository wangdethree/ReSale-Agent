...
V1 支持数码产品、图书、小家电三类商品，V2 已扩展服装类。项目不会登录或操作真实二手平台，也不会承诺估价等于真实成交价。

## 功能范围

- FastAPI 后端接口与 Streamlit 前端演示。
- SQLite 本地模拟商品库，初始化后包含数码、图书、小家电、服装四类模拟数据。
...
测试覆盖字段检查、估价规则、相似商品检索、一条完整 API 链路和四个固定演示案例。测试数据库会放在临时目录，不会污染 `data/resale.db`。
