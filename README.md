...
V1 支持数码产品、图书、小家电三类商品，V2 已扩展服装、家具和鞋包类，V3 增加本地库存和发布后表现反馈。项目不会登录或操作真实二手平台，也不会承诺估价等于真实成交价。
...
测试覆盖字段检查、估价规则、相似商品检索、本地图片线索、库存和发布表现、一条完整 API 链路和六个固定演示案例。测试数据库会放在临时目录，不会污染 `data/resale.db`。
